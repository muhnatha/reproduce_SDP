...
class ParallelLinear(torch.autograd.Function):
    """
    A computation-efficient implementation of multiple linear layers in parallel.
    This is used to implement the expert layers in Mixture of Experts.
    """
    @staticmethod
    @custom_fwd 
    def forward(ctx, input: Tensor, expert_size: Tensor, weight: Tensor, bias: Optional[Tensor]):
    ...
    @staticmethod
    @torch.jit.script # compiles into TorchScript for efficiency (parallel execution)
    def forward_scriptable(input: Tensor, expert_size: Tensor, weight: Tensor, bias: Optional[Tensor]):
        """Forward pass for parallel linear layer."""
    ...
    @staticmethod
    @torch.jit.script
    def backward_scriptable(grad_out: Tensor, input: Tensor, expert_size: Tensor, weight: Tensor, bias: Optional[Tensor]):
        """Backward pass for parallel linear layer."""
        num_linears = weight.size(0)
        expert_size_list : List[int] = expert_size.tolist()
        input_list = input.t().split(expert_size_list, dim=1)
        grad_list = grad_out.split(expert_size_list, dim=0)

        d_input_but = torch.empty_like(input)
        d_input_buf_list = d_input_but.split(expert_size_list, dim=0)
...
def compute_gating(k: int, probs: torch.Tensor, top_k_gates: torch.Tensor, top_k_indices: torch.Tensor):
    """Compute gating outputs for Mixture of Experts."""
    zeros = torch.zeros_like(probs)
    gates = zeros.scatter(1, top_k_indices, top_k_gates) # only top k are non-zero
...
class ParallelExperts(nn.Module):
    ...
    def extra_repr(self):
        return 'num_experts={}, input_size={}, output_size={}'.format(self.weight.size(0), self.weight.size(1), self.weight.size(2))
...
class MoE(nn.Module):
    ...
    def __init__(self, input_size, head_size, num_experts, k,
        ...
        self.bias = bias
        self.experts = ParallelExperts(num_experts, input_size, head_size, bias=bias)
        self.output_experts = ParallelExperts(num_experts, head_size, input_size, bias=bias)
        ...
        if True:
            ...
            self.f_gate = nn.Sequential(
                nn.Linear(input_size, 2*num_experts if noisy_gating else num_experts, bias=False),
            )
    ...
    def update_aux_statistics(self, logits, probs, gates):
        lsesq = torch.log(torch.exp(logits).sum(dim=1) + 1e-6)**2
        ...
        self.acc_lesesq_count = self.acc_lsesq_count + lsesq.size(0)
    ...
    def get_aux_loss_and_clear(self):
        ...
        zloss = self.acc_lsesq / (self.acc_lesesq_count)
        loss = (self.cvloss * cvloss + self.switchloss * switchloss + self.zloss * zloss)
        self.init_aux_statistics()
        return loss
    ...
    def compute_switchloss(self, probs, freqs):
        loss = F.normalize(probs.sum(0), p=1, dim=0) * F.normalize(freqs.float(), p=1, dim=0)
    ...
    def top_k_gating(self, x, skip_mask=None, sample_topk=0, noise_epsilon=1e-2):
        ...
        Args:
            x: input Tensor
            skip_mask: a binary mask indicating which experts to skip
            sample_topk: if >0, randomly sample top k experts from top sample_topk experts
            noise_epsilon: float, noise epsilon for noisy gating
        Returns:
    ...
    def forward(self, x, skip_mask=None, sample_topk=0, multiply_by_gates=True):
        """Forward pass for Mixture of Experts layer.
        Args:
            x: input Tensor
            skip_mask: a binary mask indicating which experts to skip
            sample_topk: if >0, randomly sample top k experts from top sample_topk experts
            multiply_by_gates: whether to multiply the expert outputs by the gating values
        Returns:
            y: output Tensor
            loss: auxiliary loss for load balancing"""
        ...
        zeros = torch.zeros((batch_size * length, self.input_size), device=expert_outputs.device, dtype=expert_outputs.dtype)
        y = zeros.index_add(0, self.batch_index, expert_outputs)
        y = y.view(batch_size, length, self.input_size)
        return y, loss
    ...
    def map(self, x, skip_mask=None, sample_topk=0):
        ...
        zeros = torch.zeros((batch_size * length * self.k, self.head_size), device=expert_outputs.device, dtype=expert_outputs.dtype)
        y = zeros.index_add(0, self.index_sorted_experts, expert_outputs)
        y = y.view(batch_size, length, self.k, -1)
        return y, loss
    
    def reduce(self, x, multiply_by_gates=True):
        """Args:
            x: tensor shape [batch_size, k, head_size]
        Returns:
            y: a tensor with shape [batch_size, output_size].
        """
        batch_size, length, k, emb_size = x.size()
        x = x.view(-1, emb_size)
...
class TaskMoE(MoE):
    ...
    def get_MIloss(self, logits, probs, gates, task_bh):
        """Compute the mutual information loss for the gating probabilities. """
        ...
        PT = 1/self.task_num # each task has the same weight
        self.PTE[task_bh] = self.PTE[task_bh] * self.momentum + (1-self.momentum) * (probs.mean(0).detach()*PT)
        loss = -self.w_H * (probs * torch.log(probs + 0.0001)).sum(1).mean() # maximize entropy
