...
class DiffusionTransformerHybridImagePolicy(BaseImagePolicy):
    def __init__(
        ...
        assert len(action_shape) == 1, "only 1D action supported"
        ...
        if obs_encoder_group_norm:
            # replace batch norm with group nor
            replace_submodules(
    ...
    def conditional_sample(
            ...
            **kwargs):
        """ Robot thinks and plans its movement"""
        model = self.model
        scheduler = self.noise_scheduler

        # define trajectory as a random gaussian noises
        ...
        # set step as value for denoising to know how noisy it is (t)
        ...
        # make sure conditioning is enforeced
        trajectory[condition_mask] = condition_data[condition_mask]
    ...
    def compute_loss(self, batch, task_id):
        ...
        # sample noise that we will add to the images
        ...
        # add noise to the trajectory according to the noise magnitude at each timestep
