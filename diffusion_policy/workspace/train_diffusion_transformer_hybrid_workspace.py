...
class TrainDiffusionTransformerHybridWorkspace(BaseWorkspace):
    ...
    def run(self):
        ...
        for dataset in datasets:
            ...
            normalizer.append(dataset.get_normalizer())
        max_train_dataloader_len = max([len(train_dataloaders) for train_dataloader in train_dataloaders])
        ...
        with JsonLogger(log_path) as json_logger:
            for local_epoch_idx in range(cfg.training.num_epochs):
                step_log = dict()
                train_losses = list()
                with tqdm.tqdm(multi_traindataloader, desc=f"Epoch {self.epoch}",
                    ...
                    for batch_idx, batch in enumerate(tepoch):
                        ...
                        if train_sampling_batchs[assigned_task_id] is None:
                            print("assigning train sampling batch for task ", assigned_task_id)
                ...
                for i, train_sampling_batch in enumerate(train_sampling_batchs):
                    if train_sampling_batch is None:
                        raise ValueError(f"train_sampling_batch for task {i} is None")
                ...
                # run diffusion sampling on train batch
